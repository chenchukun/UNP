...
#define PORT "6180" // the port client will be connecting to
...
void* do_connect(void *arg)
{
    ...
    socks = (int*)malloc(sizeof(int)*g_noverwrap);
    ...
    for (i = 0; i < g_nloop; ++i) {
        ...
        for (j=0; j<g_nhello; ++j) {
            ...
            for (k=0; k<g_noverwrap; ++k) {
                sockfd = socks[k];
                send(sockfd, "123456789012345678901234567890123456789012345678901234567890123\n", 64, 0);
            }
            for (k=0; k<g_noverwrap; ++k) {
                sockfd = socks[k];
...
int main(int argc, char *argv[])
{
    ...
    long long time_consumed;
    {
        ...
        int i;
        for (i = 0; i < nthread; ++i) {
            rv = pthread_create(&threads[i], NULL, do_connect, (void*)servinfo);
            if (rv == -1) {
